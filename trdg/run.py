...
def main():
    ...
    if args.dict:
        ...
        if os.path.isfile(args.dict):
            ...
            # with open(args.dict, "r", encoding="ms949", errors="ignore") as d:
...
if __name__ == "__main__":
    ...
    # 실행 방법 : venv/bin/python TextRecognitionDataGenerator/trdg/run.py -l kr ...

    # dict 기반 generation
    # (language) -l: kr
    # (blur) -bl: 0,1,2
    # (backgrournd) -b: 0 (gaussian noise), 1 (plane white), 2 (quasicrystal)
    # (length) -w: 1
    # (character_spacing) -cs: 측정필요 (defalt=0)
    # (img_h) -f: 측정필요 (defalt=32)
    # (count) -c: 1,000,000
    # (text_color) -tc: '#000000,#888888'
    # (dict) -dt:TextRecognitionDataGenerator/trdg/dicts/custom/kr
    # (font_dir) -fd: TextRecognitionDataGenerator/trdg/fonts/kr
